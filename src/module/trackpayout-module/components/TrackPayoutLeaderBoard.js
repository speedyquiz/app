...
const TrackPayoutLeaderBoard = ({ isOpen, onClose, _id }) => {
    //
    ...
    useEffect(() => {
        setIsLoading(true)
        fetchUserDataForPayout(currentPage)

    }, [_id])
    const fetchUserDataForPayout = (pageNoVal) => {
        dispatch(fetchAllLeaderBoardDataReq({
            reqData: {
                'id': _id._id,
                'pageNo': pageNoVal
            },
            onSuccessData: (responseData) => {
                ...
                if (pageNoVal === 1) {
                    ...
                    setCurrentPage(pageNoVal)
                    setPlayerData([...playerData, ...responseData?.data]);
                }
            },
            onErrorData: (error) => {
                setIsLoading(false)
    ...
    const handleChangePage = (event, newPage) => {
        ...
        if (currentPage < (newPage + 1)) {
            setIsLoading(true)
            fetchUserDataForPayout((newPage + 1));
        }
    };
    //
...
export default TrackPayoutLeaderBoard;
